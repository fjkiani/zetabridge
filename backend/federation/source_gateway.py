...
class EgaClient:
    ...
    # Server-side authenticated byte transfer over the v2 Data API (port 8443,
    # reachable from Render). Credentials stay in cfg/env and are never returned
    # to the caller. We reuse the pyega3 client (token flow + retry + correct v2
    # host) rather than hand-rolling OAuth + slicing. NOTE: pyega3 v2 targets
    # 8443, NOT the firewall-blocked legacy 8052 path.
    ...
    def download_size(self, file_id: str) -> Envelope:
        """Authenticated file-size lookup via the v2 Data API (proves auth works)."""
    ...
    def iter_file_bytes(self, file_id: str, chunk_size: int = 8 * 1024 * 1024):
        """Yield decrypted file bytes for ``file_id`` from the v2 EGA Data API.
